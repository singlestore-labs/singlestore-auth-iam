...
public interface CloudProviderClient {
    ...
    Exception fastDetect();       // Fast in-process detection only; returns null if detected; else an exception.
    ...
    class IdentityHeadersResult {
        ...
        public IdentityHeadersResult(Map<String, String> headers, CloudIdentity identity, Exception error) {
            this.headers = headers; this.identity = identity; this.error = error;
        }
    }
}
