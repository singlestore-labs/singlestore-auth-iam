...
public final class S2IAM {
    ...
    public static String getDatabaseJWT(String workspaceGroupId, JwtOption... opts) throws S2IAMException {
    ...
    // Provider detection
    public static CloudProviderClient detectProvider(ProviderOption... opts) throws NoCloudProviderDetectedException {
        ...
        if (po.clients == null) {
            po.clients = List.of(new AWSClient(po.logger), new GCPClient(po.logger), new AzureClient(po.logger));
        }
        ...
        for (CloudProviderClient c : po.clients) {
            futures.add(cs.submit(() -> {
                ...
                if (err == null) return c; else throw err;
            }));
        }
        exec.shutdown();
        ...
        throw new NoCloudProviderDetectedException("no cloud provider detected" + (errors.isEmpty() ? "" : (": " + errors.get(0).getMessage())));
    }
    ...
    private static String getJWT(JwtOptions o) throws S2IAMException {
        ...
        String url = o.serverUrl
            .replace(":cloudProvider", identity.getProvider().name())
            .replace(":jwtType", o.jwtType.name());

        String query = "";
        if (o.jwtType == JwtOptions.JWTType.database && o.workspaceGroupId != null && !o.workspaceGroupId.isEmpty()) {
        ...
        if (response.statusCode() != 200) {
            throw new S2IAMException("authentication server returned status " + response.statusCode() + ": " + response.body());
        }
        try {
