...
/**
 * Local mock provider used only for Java integration tests with Go server when no cloud metadata
 * present.
 */
class LocalMockProvider implements CloudProviderClient {
  ...
  public IdentityHeadersResult getIdentityHeaders(Map<String, String> additionalParams) {
    // Provide minimal headers that Go verifier will accept for AWS path: access key + secret.
    Map<String, String> h =
        Map.of(
            "X-AWS-Access-Key-ID", "TESTACCESSKEY",
