...
/**
 * Parity test: when S2IAM_TEST_ASSUME_ROLE is set we must both detect the cloud provider and
 * successfully assume the specified role. Mirrors Go test expectations (fail-fast instead of skip).
 */
public class S2IAMAssumeRoleTest {
  ...
  void testAssumeRoleMustSucceedWhenEnvSet() {
    ...
    try {
      ...
      fail(
          "Expected provider detection to succeed when S2IAM_TEST_ASSUME_ROLE set: "
              + e.getMessage());
      return; // unreachable
    }
