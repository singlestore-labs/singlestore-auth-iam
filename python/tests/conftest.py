...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "requires_cloud: marks tests that require real cloud environment"
    )
    ...
    try:
        ...
        if loop.is_running():
            ...
            env_indicators = [
                ...
                # GCP  
                'GOOGLE_CLOUD_PROJECT', 'GCP_PROJECT', 'GCE_METADATA_HOST',
                # Azure
                'AZURE_CLIENT_ID', 'MSI_ENDPOINT', 'IDENTITY_ENDPOINT'
            ]
            ...
            return (_check_aws_metadata() or 
                    _check_gcp_metadata() or 
                    _check_azure_metadata())
    except RuntimeError:
